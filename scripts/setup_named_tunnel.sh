...
#
# Option A — Zero Trust tunnel token (recommended; no cert.pem on disk):
...
#   4. Copy configs/cloudflared/config.yml.example → ~/.cloudflared/config.yml
#      and fill in tunnel id + credentials path.
#
set -euo pipefail
